...
void reply_put(accepted_socket& client_sock, cix_header& header) {
   ...
   ofstream content(header.filename, ofstream::binary);
   if (content) {
      header.command = cix_command::ACK;
      content.write(buffer, header.nbytes);
   }else{
...
void reply_get(accepted_socket& client_sock, cix_header& header) {
   std::ifstream content(header.filename, std::ifstream::binary);
   if(content){
      content.seekg(0, content.end);
      int len = content.tellg();
      content.seekg(0, content.beg);
      char buffer[len];
      content.read(buffer, len); 
      header.nbytes = len;
